...
#make stimulus list for trial block 1
...
# finding csv directory and writing in the shuffled list
list = 'shuffled_list1.csv'
path = str(os.path.abspath(list))
f = open(path, 'w')
writer = csv.writer(f)
...
#make stimulus list for trial block 2
stimulus_list = ['there', 'think', 'about', 'would', 'lucky', 'buddy', 'cheer', 'theme', 'belly', 'champ', 'puppy', 'vodka', 'tithe', 'therm', 'haole', 'cooed', 'faqir', 'druze', 'edthe', 'heond', 'kremp', 'isvar', 'zibja', 'pykka'] * 5
shuffle(stimulus_list)
...
#make stimulus list for trial block 3
stimulus_list = ['there', 'think', 'about', 'would', 'lucky', 'buddy', 'cheer', 'theme', 'belly', 'champ', 'puppy', 'vodka', 'tithe', 'therm', 'haole', 'cooed', 'faqir', 'druze', 'edthe', 'heond', 'kremp', 'isvar', 'zibja', 'pykka'] * 5
shuffle(stimulus_list)
...
#make stimulus list for trial block 4
stimulus_list = ['there', 'think', 'about', 'would', 'lucky', 'buddy', 'cheer', 'theme', 'belly', 'champ', 'puppy', 'vodka', 'tithe', 'therm', 'haole', 'cooed', 'faqir', 'druze', 'edthe', 'heond', 'kremp', 'isvar', 'zibja', 'pykka'] * 5
shuffle(stimulus_list)
...
expName = 'test_typingtask_norepeats_20rep'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/rubi/Desktop/Action Control Lab/typing_task/20rep_test/test_typingtask_norepeats_20rep.py',
...
sound_1A_3 = sound.Sound('A', secs=0.1, stereo=True, hamming=True,
    name='sound_1A_3')
sound_1A_3.setVolume(1)
...
sound_1A_4 = sound.Sound('A', secs=0.1, stereo=True, hamming=True,
    name='sound_1A_4')
sound_1A_4.setVolume(1)
...
for thisPractice_trial in practice_trials:
    ...
    # keep track of which components have finished
    practice_trialComponents = [text, key_resp_0, sound_1A]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if key_resp_0.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp_0.tStartRefresh + 3.5-frameTolerance:
                # keep track of stop time/frame for later
                key_resp_0.tStop = t  # not accounting for scr refresh
                key_resp_0.frameNStop = frameN  # exact frame index
                win.timeOnFlip(key_resp_0, 'tStopRefresh')  # time at next scr refresh
                key_resp_0.status = FINISHED
        if key_resp_0.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_0.getKeys(keyList=['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'space', 'enter', 'backspace'], waitRelease=False)
            _key_resp_0_allKeys.extend(theseKeys)
            if len(_key_resp_0_allKeys):
                key_resp_0.keys = [key.name for key in _key_resp_0_allKeys]  # storing all keys
                key_resp_0.rt = [key.rt for key in _key_resp_0_allKeys]
                # was this correct?
                if corrAns is not None and ((''.join(key_resp_0.keys) == str(corrAns)) or (key_resp_0.keys == corrAns)):
                    key_resp_0.corr = 1
                else:
                    key_resp_0.corr = 0
        # start/stop sound_1A
    ...
    if key_resp_0.keys in ['', [], None]:  # No response was made
        ...
        if str(corrAns).lower() == 'none':
           key_resp_0.corr = 1;  # correct non-response
        else:
           key_resp_0.corr = 0;  # failed to respond (incorrectly)
    # store data for practice_trials (TrialHandler)
    practice_trials.addData('key_resp_0.keys',key_resp_0.keys)
    practice_trials.addData('key_resp_0.corr', key_resp_0.corr)
    if key_resp_0.keys != None:  # we had a response
        practice_trials.addData('key_resp_0.rt', key_resp_0.rt)
    ...
    # update component parameters for each repeat
    if key_resp_0.corr:#stored on last run routine
      msg="Correct!"
      feedback_2.color='limegreen'
    else:
      msg="Oops! That was wrong"
      feedback_2.color='orangered'
...
for thisTrial_1 in trials_1:
        ...
        if key_resp_1.status == STARTED and not waitOnFlip:
            ...
            if len(_key_resp_1_allKeys):
                ...
                key_resp_1.rt = [key.rt for key in _key_resp_1_allKeys]
                 # was this correct?
                if corrAns is not None and ((''.join(key_resp_1.keys) == str(corrAns)) or (key_resp_1.keys == corrAns)):
                    key_resp_1.corr = 1
                else:
                    key_resp_1.corr = 0
    ...
    routineTimer.add(1.000000)
    # update component parameters for each repeat
    if key_resp_1.corr:#stored on last run routine
      msg="Correct!"
      feedback_2.color='limegreen'
    else:
      msg="Oops! That was wrong"
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if key_resp_2.status == STARTED and not waitOnFlip:
            ...
            if len(_key_resp_2_allKeys):
                ...
                # was this correct?
                if corrAns is not None and ((''.join(key_resp_2.keys) == str(corrAns)) or (key_resp_2.keys == corrAns)):
                    key_resp_2.corr = 1
                else:
                    key_resp_2.corr = 0
...
trials_3 = data.TrialHandler(nReps=5, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('shuffled_list3.csv'),
...
for thisTrial_3 in trials_3:
    ...
    routineTimer.add(3.500000)
    # update component parameters for each repeat
    text_3.setText(string)
    key_resp_3.keys = []
    key_resp_3.rt = []
    _key_resp_3_allKeys = []
    sound_1A_3.setSound('A', secs=0.1, hamming=True)
    sound_1A_3.setVolume(1, log=False)
    # keep track of which components have finished
    trial_3Components = [text_3, key_resp_3, sound_1A_3]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if key_resp_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp_3.tStartRefresh + 3.5-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if key_resp_3.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_3.getKeys(keyList=['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'space', 'enter', 'backspace'], waitRelease=False)
            ...
            if len(_key_resp_3_allKeys):
                ...
                # was this correct?
                if corrAns is not None and ((''.join(key_resp_3.keys) == str(corrAns)) or (key_resp_3.keys == corrAns)):
                    key_resp_3.corr = 1
                else:
                    key_resp_3.corr = 0
        # start/stop sound_1A_3
        if sound_1A_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            sound_1A_3.frameNStart = frameN  # exact frame index
            sound_1A_3.tStart = t  # local t and not account for scr refresh
            sound_1A_3.tStartRefresh = tThisFlipGlobal  # on global time
            sound_1A_3.play(when=win)  # sync with win flip
        if sound_1A_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_1A_3.tStartRefresh + 0.1-frameTolerance:
                # keep track of stop time/frame for later
                sound_1A_3.tStop = t  # not accounting for scr refresh
                sound_1A_3.frameNStop = frameN  # exact frame index
                win.timeOnFlip(sound_1A_3, 'tStopRefresh')  # time at next scr refresh
                sound_1A_3.stop()
    ...
    thisExp.nextEntry()
    sound_1A_3.stop()  # ensure sound has stopped at end of routine
    thisExp.addData('sound_1A_3.started', sound_1A_3.tStartRefresh)
    thisExp.addData('sound_1A_3.stopped', sound_1A_3.tStopRefresh)
    
    # ------Prepare to start Routine "feedback"-------
    continueRoutine = True
    routineTimer.add(1.000000)
    # update component parameters for each repeat
    if key_resp_3.corr:#stored on last run routine
      msg="Correct!"
      feedback_2.color='limegreen'
    else:
      msg="Oops! That was wrong"
...
trials_4 = data.TrialHandler(nReps=5, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('shuffled_list4.csv'),
...
for thisTrial_4 in trials_4:
    ...
    routineTimer.add(3.500000)
    # update component parameters for each repeat
    text_4.setText(string)
    key_resp_4.keys = []
    key_resp_4.rt = []
    _key_resp_4_allKeys = []
    sound_1A_4.setSound('A', secs=0.1, hamming=True)
    sound_1A_4.setVolume(1, log=False)
    # keep track of which components have finished
    trial_4Components = [text_4, key_resp_4, sound_1A_4]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if key_resp_4.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp_4.tStartRefresh + 3.5-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if key_resp_4.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_4.getKeys(keyList=['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'space', 'enter', 'backspace'], waitRelease=False)
            ...
            if len(_key_resp_4_allKeys):
                ...
                # was this correct?
                if corrAns is not None and ((''.join(key_resp_4.keys) == str(corrAns)) or (key_resp_4.keys == corrAns)):
                    key_resp_4.corr = 1
                else:
                    key_resp_4.corr = 0
        # start/stop sound_1A_4
        if sound_1A_4.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            sound_1A_4.frameNStart = frameN  # exact frame index
            sound_1A_4.tStart = t  # local t and not account for scr refresh
            sound_1A_4.tStartRefresh = tThisFlipGlobal  # on global time
            sound_1A_4.play(when=win)  # sync with win flip
        if sound_1A_4.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_1A_4.tStartRefresh + 0.1-frameTolerance:
                # keep track of stop time/frame for later
                sound_1A_4.tStop = t  # not accounting for scr refresh
                sound_1A_4.frameNStop = frameN  # exact frame index
                win.timeOnFlip(sound_1A_4, 'tStopRefresh')  # time at next scr refresh
                sound_1A_4.stop()
    ...
    thisExp.nextEntry()
    sound_1A_4.stop()  # ensure sound has stopped at end of routine
    thisExp.addData('sound_1A_4.started', sound_1A_4.tStartRefresh)
    thisExp.addData('sound_1A_4.stopped', sound_1A_4.tStopRefresh)
    
    # ------Prepare to start Routine "feedback"-------
    continueRoutine = True
    routineTimer.add(1.000000)
    # update component parameters for each repeat
    if key_resp_4.corr:#stored on last run routine
      msg="Correct!"
      feedback_2.color='limegreen'
    else:
      msg="Oops! That was wrong"
