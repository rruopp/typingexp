...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/rubi/Downloads/typing_task/typingtask_V2.py',
...
instructions = visual.TextStim(win=win, name='instructions',
    text="During this task you will see strings of text displayed to you. Once the text appears, that is your cue to type the presented text as quickly and as accurately as possible.\n\nWhen you are done typing no need to press enter, the next trial will automatically begin. Backspacing will also have no effect, so if you make a mistake, keep typing.\n\nLet's run through a practice round first.\nPress any key to continue!",
    font='Arial',
...
reset = visual.TextStim(win=win, name='reset',
    text='Reset your hands to the home position. ',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
practice_trials = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('typingtask_variables_withcorrAns.xls', selection='1,2,3'),
...
for thisPractice_trial in practice_trials:
    ...
    # keep track of which components have finished
    trialComponents = [text, key_resp, sound_1A]
   ...
   # ------Prepare to start Routine "feedback"-------
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *reset* updates
        if reset.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            reset.frameNStart = frameN  # exact frame index
            reset.tStart = t  # local t and not account for scr refresh
            reset.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(reset, 'tStartRefresh')  # time at next scr refresh
            reset.setAutoDraw(True)
        if reset.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > reset.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
                reset.tStop = t  # not accounting for scr refresh
                reset.frameNStop = frameN  # exact frame index
                win.timeOnFlip(reset, 'tStopRefresh')  # time at next scr refresh
                reset.setAutoDraw(False)
    ...
    for thisComponent in hand_resetComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    practice_trials.addData('reset.started', reset.tStartRefresh)
    practice_trials.addData('reset.stopped', reset.tStopRefresh)
...
trials = data.TrialHandler(nReps=5, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('typingtask_variables_withcorrAns.xls'),
...
for thisTrial in trials:
    ...
    for thisComponent in hand_resetComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('reset.started', reset.tStartRefresh)
    trials.addData('reset.stopped', reset.tStopRefresh)
    thisExp.nextEntry()
...
for thisTrial_2 in trials_2:
   ...
   # update component parameters for each repeat
    ...
    for thisComponent in hand_resetComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_2.addData('reset.started', reset.tStartRefresh)
    trials_2.addData('reset.stopped', reset.tStopRefresh)
    thisExp.nextEntry()
