...
This experiment was created using PsychoPy3 Experiment Builder (v2020.2.10),
    on Tue 08 Feb 2022 10:08:43 PM PST
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/home/rubi/Downloads/typing_task_V2/typing_task_V2/test_typingtask_norepeats.py',
...
f = open('/home/rubi/Downloads/typing_task_V2/typing_task_V2/shuffled_list1.csv', 'w')
writer = csv.writer(f)
...
f = open('/home/rubi/Downloads/typing_task_V2/typing_task_V2/shuffled_list2.csv', 'w')
writer = csv.writer(f)
...
for thisPractice_trial in practice_trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if reset_image.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > reset_image.tStartRefresh + 1.5-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in hand_resetComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    practice_trials.addData('reset_image.started', reset_image.tStartRefresh)
    practice_trials.addData('reset_image.stopped', reset_image.tStopRefresh)
    thisExp.nextEntry()
...
for thisTrial_1 in trials_1:
    ...
    # keep track of which components have finished
    trial_1Components = [text_2, key_resp_2, sound_1A_2]
    ...
    for thisComponent in trial_1Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_1.addData('text_2.started', text_2.tStartRefresh)
    trials_1.addData('text_2.stopped', text_2.tStopRefresh)
    # check responses
    ...
    trials_1.addData('key_resp_2.corr', key_resp_2.corr)
    if key_resp_2.keys != None:  # we had a response
        trials_1.addData('key_resp_2.rt', key_resp_2.rt)
    trials_1.addData('key_resp_2.started', key_resp_2.tStartRefresh)
    trials_1.addData('key_resp_2.stopped', key_resp_2.tStopRefresh)
    sound_1A_2.stop()  # ensure sound has stopped at end of routine
    trials_1.addData('sound_1A_2.started', sound_1A_2.tStartRefresh)
    trials_1.addData('sound_1A_2.stopped', sound_1A_2.tStopRefresh)
    ...
    for thisComponent in hand_resetComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_1.addData('reset_image.started', reset_image.tStartRefresh)
    trials_1.addData('reset_image.stopped', reset_image.tStopRefresh)
    thisExp.nextEntry()
...
for thisTrial_2 in trials_2:
    ...
    # keep track of which components have finished
    trial_2Components = [text_3, key_resp_3, sound_1A_3]
    ...
    trials_2.addData('sound_1A_3.started', sound_1A_3.tStartRefresh)
    trials_2.addData('sound_1A_3.stopped', sound_1A_3.tStopRefresh)
    ...
    for thisComponent in hand_resetComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_2.addData('reset_image.started', reset_image.tStartRefresh)
    trials_2.addData('reset_image.stopped', reset_image.tStopRefresh)
    thisExp.nextEntry()
