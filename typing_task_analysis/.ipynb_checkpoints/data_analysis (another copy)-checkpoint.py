...
df = pd.read_csv('Ian_w_video_typingtask_2020_Dec_02_2137.csv')
...
plt.scatter(
    ...
    c=['red'] * len(mostfam_delay) + ['blue'] * len(semifam_delay) + ['green'] * len(unfam_delay) + ['orange'] * len(
        nofam_delay)
)
...
plt.scatter(
    ...
    c=['red'] * len(mostfam_speed) + ['blue'] * len(semifam_speed) + ['green'] * len(unfam_speed) + ['orange'] * len(
        nofam_speed)
)
...
def inner_word_variance(word, trials):
    ...
    return [ np.ma.var(x) for x in letter_dilats ]
...
print(sequence_variance_averages(mostfam_trials))
...
# mostfam_innervar = inner_word_variance(('there', 'about', 'right', 'think', 'would', 'where'), mostfam_trials)
# print(mostfam_innervar)

# mostfam_dilat_var = list(map(lambda trial: np.ma.var(trial['dilat']), mostfam_dilat))
# print(mostfam_dilat_var)
#
# mostfam_dilat_varavg = np.mean(mostfam_dilat_var)
# print(mostfam_dilat_varavg)
#
# semifam_dilat = list(
#     map(lambda trial: {'string': trial['string'], 'dilat': np.diff(trial['key_resp.rt'])}, semifam_trials))
# semifam_dilat_var = list(map(lambda trial: np.ma.var(trial['key_resp.rt']), semifam_trials))
# semifam_dilat_varavg = np.mean(semifam_dilat_var)
# print(semifam_dilat_varavg)
#
# unfam_dilat = list(map(lambda trial: {'string': trial['string'], 'dilat': np.diff(trial['key_resp.rt'])}, unfam_trials))
# unfam_dilat_var = list(map(lambda trial: np.ma.var(trial['key_resp.rt']), unfam_trials))
# unfam_dilat_varavg = np.mean(unfam_dilat_var)
# print(unfam_dilat_varavg)
#
# nofam_dilat = list(map(lambda trial: {'string': trial['string'], 'dilat': np.diff(trial['key_resp.rt'])}, nofam_trials))
# nofam_dilat_var = list(map(lambda trial: np.ma.var(trial['key_resp.rt']), nofam_trials))
# nofam_dilat_varavg = np.mean(nofam_dilat_var)
# print(nofam_dilat_varavg)

# # plotting digraph latency variations
fig = plt.figure(figsize=(10, 7))
ax = fig.add_subplot(111)
bp = ax.boxplot([mostfam_dilat_var, semifam_dilat_var, unfam_dilat_var, nofam_dilat_var])
...
plt.ylabel("Digraph Latency Variation")
plt.title("Digraph Latency Variation by String Type")
#
# # plt.show()
...
plt.figure(figsize=(10, 7))
plt.bar(['mostfam', 'semifam', 'unfam', 'nofam'],
        [len(mostfam_totalerror), len(semifam_totalerror), len(unfam_totalerror), len(nofam_totalerror)], color='blue',
        width=0.4)
